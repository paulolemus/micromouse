 ...
 *
 * Description:
 *     Unit tests to ensure that floodfill is working as expected.
 *
 * Notes:
 *
 *
...
START_TEST(check_floodfill_empty_maze) {
    ...
    ck_assert_uint_eq(init_maze(&maze, width, height), 1);
    box_maze(&maze);
...
// Check floodfil values on maze with a single wall that shouldn't block the
// flood.
START_TEST(check_floodfill_nonblocking_walls) {
    ...
    set_field_on(&maze, 1, 1, WALL_SOUTH);
